...
#include "../SaxPac/IndependentSetRectangles.h"

using namespace std;

EffectiveGrid::EffectiveGrid(const vector<Rule>& rules, int dim) : dim(dim) {
	vector<unsigned int> lows = sp::ConvertDistinct<Rule, unsigned int>(rules, [=](const Rule& r) { return r.range[dim][LowDim]; });
	vector<unsigned int> highs = sp::ConvertDistinct<Rule, unsigned int>(rules, [=](const Rule& r) { return r.range[dim][HighDim]; });
...
void EffectiveGrid::SortableSplit(const vector<Rule>& rules, const vector<int>& dimOrder, vector<Rule>& receivingList, size_t dimIndex) {
	if (dimIndex >= dimOrder.size()) {
		...
		// Group rules on dim
		vector<unsigned int> lows = sp::ConvertDistinct<Rule, unsigned int>(rec, [=](const Rule& r) {return r.range[dim][LowDim]; });
